# Resources for learning machine learning

<p float="left">
  <p style="font-weight: bold; margin: 0 0 8px 0; font-size: 18px;">xkcd comic: Machine Learning Captcha</p>
  <img
    src="https://imgs.xkcd.com/comics/machine_learning_captcha.png"
    height="485"
    alt="xkcd comic: Machine Learning Captcha"
    title="More likely: Click on all the pictures of people who appear disloyal to [name of company or government]"
  />
  <figcaption style="margin-top: 8px; font-size: 14px;">
    Source: <a href="https://xkcd.com/2228/" target="_blank" rel="noopener noreferrer">xkcd.com/2173</a>
  </figcaption>
</p>

## Books
* Bishop, Christopher M. (2006). [Pattern Recognition and Machine Learning](https://www.microsoft.com/en-us/research/wp-content/uploads/2006/01/Bishop-Pattern-Recognition-and-Machine-Learning-2006.pdf). Springer. `Biblioteca Carlos Enrique Ruíz 006.4/B622p`
* Bishop, Christopher M., and Bishop, Hugh (2024). Deep learning: foundations and concepts. Springer. The book can be read online at https://www.bishopbook.com/. 
* Deisenroth, Marc Peter; Faisal, A. Aldo; and Ong, Cheng Soon (2020). [Mathematics for Machine Learning](https://mml-book.github.io/book/mml-book.pdf). Cambridge University Press. https://mml-book.github.io/ `Biblioteca Carlos Enrique Ruíz 512.00285/D325m`
* James, Gareth; Witten, Daniela; Hastie, Trevor; Tibshirani, Robert; and Taylor Jonathan (2023). [An Introduction to Statistical Learning with applications in Python](https://www.statlearning.com/). Springer. `Biblioteca Carlos Enrique Ruíz 519.5/J272i (R version, 2014)`
* Goodfellow, Ian; Bengio, Yoshua; and Courville, Aaron (2016). [Deep Learning](https://www.deeplearningbook.org/). MIT Press, http://www.deeplearningbook.org. See also: https://github.com/janishar/mit-deep-learning-book-pdf `Biblioteca Alfonso Carvajal Escobar 006.31/G651d`
* Géron, Aurélien (2022). [Hands-on machine learning with scikit-learn and TensorFlow: concepts, tools, and techniques to build intelligent systems.]((https://ageron.github.io/)). O'Reilly. `Biblioteca Alfonso Carvajal Escobar 006.31/G377h`
* Géron, Aurélien (2025). [Hands-On Machine Learning with Scikit-Learn and PyTorch](https://ageron.github.io/). O'Reilly.
* Goulet, James A. (2020). [Probabilistic machine learning for civil engineers](https://profs.polymtl.ca/jagoulet/Site/Goulet_web_page_BOOK.html). The MIT Press. `Biblioteca Alfonso Carvajal Escobar 624.0285/G698p`
* Janapa Reddi, Vijay (2026). [Machine Learning Systems: Principles and Practices of Engineering Artificially Intelligent Systems.](https://mlsysbook.ai/book/assets/downloads/Machine-Learning-Systems.pdf) https://github.com/harvard-edge/cs249r_book
* Murphy, Kevin P. (2025, April 18). [Probabilistic Machine Learning: An introduction](https://github.com/probml/pml-book/releases/latest/download/book1.pdf), MIT Press, http://probml.github.io/book1
* Prince, Simon J.D. (2025, May 29). [Understanding Deep Learning](https://github.com/udlbook/udlbook/releases/download/v5.0.2/UnderstandingDeepLearning_05_29_25_C.pdf). The MIT Press, http://udlbook.com
* Raschka, Sebastian; Liu, Yuxi (Hayden); and Mirjalili, Vahid (2022). [Machine Learning with PyTorch and Scikit-Learn: Develop machine learning and deep learning models with Python](https://darajat.ly/wp-content/uploads/2024/06/Machine-Learning-With-PyTorch-and-Scikit-Learn.pdf). Packt Publishing, Birmingham, UK. https://github.com/rasbt/machine-learning-book `Biblioteca Alfonso Carvajal Escobar 006.31/R223m`
* Theodoridis, Sergios (2026). Machine Learning: From the Classics to Deep Networks, Transformers, and Diffusion Models. CRC Press, Taylor & Francis Group. 3th edition. `Biblioteca Alfonso Carvajal Escobar 006.31/T388m (1st edition, 2015)`
* Zhang, Aston; Lipton, Zachary C.; Li, Mu; and Smola, Alexander J. (2023). [Dive into Deep Learning](https://d2l.ai/d2l-en.pdf). Cambridge University Press, https://D2L.ai

## Blogs
* Andrej Karpathy: https://karpathy.github.io/
* Christopher Olah: https://colah.github.io/

## Youtube channels
* 3Blue1Brown: https://www.youtube.com/c/3blue1brown
* 3Blue1Brown en español: https://www.youtube.com/@3blue1brownespanol
* Pepe Cantoral: https://www.youtube.com/@PepeCantoralPhD

<!---
que bases de datos existen en Kaggle que puedan ser útiles para un curso de aprendizaje automático aplicado a la ingeniería civil

Datasets: UCI Concrete Compressive Strength, Kaggle Structural Damage Detection, USGS River Discharge.

Para un curso de Aprendizaje Automático (ML) aplicado a la Ingeniería Civil, Kaggle ofrece diversos conjuntos de datos que cubren áreas clave como materiales, gestión de proyectos y salud estructural.
Materiales y Estructuras
Estos conjuntos de datos son ideales para ejercicios de regresión y visión computacional.

    Concrete Compressive Strength: Un clásico para predecir la resistencia a la compresión del concreto basada en su composición (cemento, agua, cenizas, edad, etc.).
    Structural Defects Network (SDNET) 2018: Contiene más de 56,000 imágenes para el entrenamiento de modelos de detección de grietas en concreto.
    Bridge Defects and Objects: Un dataset de segmentación semántica con 10,000 imágenes que incluyen 13 clases de daños como grietas, corrosión y eflorescencia. 

Gestión de Proyectos y Construcción 
Útiles para enseñar análisis predictivo y procesamiento de lenguaje natural (NLP).

    Construction Estimation Data: Incluye 1,000 entradas simuladas para modelar costos de construcción considerando materiales, mano de obra y ajustes de políticas.
    BIM-AI Integrated Dataset: Diseñado para la gestión del ciclo de vida de proyectos, integrando datos de costos, cronogramas, salud estructural y riesgos de seguridad.
    Construction Site Safety Image Dataset: Enfocado en la seguridad laboral, permitiendo detectar el uso de equipo de protección personal (EPP) mediante visión computacional. 

Infraestructura y Transporte
Para aplicaciones de series temporales y análisis geográfico.

    2022 National Bridge Inventory Data: Información detallada sobre el estado y características de los puentes en EE. UU., útil para predecir necesidades de mantenimiento.
    US Road Construction and Closures: Datos sobre cierres de carreteras y actividades de construcción entre 2016 y 2021. 



--->
